...
export const numberToKorean = (number: string) => {
...
export const priceRegex = /\B(?=(\d{3})+(?!\d))/g;

export const converterPrice = (price: string): string => {
  const result = price.replace(priceRegex, ',');
  return result;
};

export const calculationDiscount = (
  price: number,
  discount: number,
): number => {
